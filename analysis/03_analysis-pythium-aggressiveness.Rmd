...
```{r fm_seed_agg_rot_5 }
fm_seed_agg_rot_5 <- glm(rotted_5 ~ species + isolate_id + rep, data = seed_soybean)
qplot(fm_seed_agg_rot_5$residuals)
resid_panel(fm_seed_agg_rot_5)
anova(fm_seed_agg_rot_5, test = "Chisq")
emmeans(fm_seed_agg_rot_5, ~ species, type = "response") %>% 
    multcomp::cld()
```
...
anova(fm_seed_corn_col_5) # remove run 
```
...
anova(fm_seed_corn_ger_5) # remove run
```
...
autoplot(fm_millet_ger, which = 1:6, ncol = 3, label.size = 3)
...
autoplot(fm_millet_weight, which = 1:6, ncol = 3, label.size = 3)
...
seedling_soybean_millet %>%
  ggplot(aes(x = reorder(species, germination), y = germination)) +
